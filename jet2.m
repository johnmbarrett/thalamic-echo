function J = jet2(m, NaNcoloroption)
% derived from Matlab's JET colormap
% this one has a black level

% 'NaNcoloroption' -- 'black' 'gray' 'white'
%JET    Variant of HSV.
...
if nargin <= 1; NaNcoloroption = 'black'; end
n = max(round(m/3.1),1);
x = (1:n)'/n;
y = (n/2:n)'/n;
e = ones(length(x),1);
...
switch NaNcoloroption
    case {'black' 'b'}
        J(1,:) = 0; % for black NaNs & infs
    case {'gray' 'grey'}
        J(1,:) = 0.5; % for black NaNs & infs
    case {'white' 'w'}
        J(1,:) = 1; % for white NaNs & infs
end